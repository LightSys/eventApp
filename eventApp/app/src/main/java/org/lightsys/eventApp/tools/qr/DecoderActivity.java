...
public class DecoderActivity extends Activity implements IDecoderActivity, SurfaceHolder.Callback {
    ...
    private final Collection<BarcodeFormat> decodeFormats = null;
    ...
    private void drawResultPoints(Bitmap barcode, Result rawResult) {
        ...
        if (points != null && points.length > 0) {
            ...
            paint.setColor(ContextCompat.getColor(getBaseContext(), R.color.result_image_border));
            paint.setStrokeWidth(3.0f);
            paint.setStyle(Paint.Style.STROKE);
            Rect border = new Rect(2, 2, barcode.getWidth() - 2, barcode.getHeight() - 2);
            canvas.drawRect(border, paint);

            paint.setColor(ContextCompat.getColor(getBaseContext(), R.color.result_points));
            if (points.length == 2) {
